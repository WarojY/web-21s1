...
const cinemaList = async (req, res) => {
  try {
    ...
    // 2. Query
    const query = db.collection('cinemas').orderBy('city').orderBy('name').get()
...
const cinemaDetails = async (req, res) => {
  try {
    ...
    const queryScreenings = db.collection('screenings').where('cinemaSlug', '==', slug).orderBy('timeString').orderBy('screen').get()
