...
const filmDetails = async (req, res) => {
  try {
    ...
    const queryScreenings = db.collection('screenings').where('filmSlug', '==', slug).where('cinemaSlug', 'in', cinemaSlugs).orderBy('timeString').orderBy('cinemaName').orderBy('screen').get()
