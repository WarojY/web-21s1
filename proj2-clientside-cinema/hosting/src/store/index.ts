...
const store = new Vuex.Store<Store>({
  ...
  getters: {
    ...
    cinemaForSlug: (_state, getters) => (cinemaSlug: string) =>
      getters.cinemasBySlug[cinemaSlug],
    filmForSlug: (_state, getters) => (filmSlug: string) =>
      getters.filmsBySlug[filmSlug],
    screeningForSlug: (_state, getters) => (screeningSlug: string) =>
      getters.screeningsBySlug[screeningSlug],
    screeningsForCinema: state => (cinemaSlug: string) =>
      state.screenings
        .filter(s => s.cinemaSlug === cinemaSlug)
        .sort((s1, s2) => s1.timeString.localeCompare(s2.timeString)),
    screeningsForFilm: (state, getters) => (filmSlug: string) =>
      state.screenings
        .filter(
          s =>
            s.filmSlug === filmSlug &&
            getters.cinemasBySlug[s.cinemaSlug].city === state.activeLocation
        )
        .sort((s1, s2) => s1.timeString.localeCompare(s2.timeString))
  },
  mutations: {
